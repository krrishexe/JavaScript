...
function fetchMovies(url, dom_element, path_type) {
    ...
    // Within F etch get the response and call showMovies() with the data , dom_element, and path type
}
...
showMovies = (movies, dom_element, path_type) => {
    ...
    // Create a variable that grabs id or class                     when we have to select between id and class at the same time , then we have to use .queryselector()

    var moviesEL = document.querySelector('dom_element');

    // Loop through object ( for of loop)
    for (let movie of movies.results) {
        ...
        moviesEL.appendChild(imageElement);                 // image element is the single single movies which we are adding to the original_movie div
    }
}
...
function getOriginals() {
    ...
    fetchMovies(url, '.origianl__movies', 'poster_path')
}
